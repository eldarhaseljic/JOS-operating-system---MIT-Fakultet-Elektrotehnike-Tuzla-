	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
  ...
  if( (uint32_t)srcva >= UTOP || (uint32_t)dstva >= UTOP 
      || (uint32_t)srcva % PGSIZE || (uint32_t)dstva % PGSIZE   ) return -E_INVAL;
  
  if( envid2env( srcenvid , &src_env , 1  ) 
      || envid2env( dstenvid , &dest_env , 1  )  ) return -E_BAD_ENV;
  
  if( !( page = page_lookup( src_env->env_pgdir , srcva , &pte  )  )  ) return -E_INVAL; 
  
  if( ( perm & PTE_W  ) && !( *pte & PTE_W  )  ) return -E_INVAL;
  
  return page_insert( dest_env->env_pgdir , page , dstva , perm  );
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
  ...
  if (env->env_ipc_dstva == (void *)-1 || srcva == (void *)-1) perm = 0;

  if (perm) 
  {
    if (srcva >= (void *)UTOP || (unsigned)srcva % PGSIZE != 0 || 
       ((~PTE_SYSCALL & perm) != 0)) return -E_INVAL;

    struct PageInfo *pp = page_lookup(curenv->env_pgdir, srcva, 0);
    if (!pp) return -E_INVAL;
    
    r = page_insert(env->env_pgdir, pp, env->env_ipc_dstva, PTE_U|perm);
    if (r < 0)return r;
  }
